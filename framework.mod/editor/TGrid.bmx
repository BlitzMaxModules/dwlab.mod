...
Type TGrid Extends LTShape
	...
	Method Settings()
		Local GridSettingsWindow:TGadget = CreateWindow( "{{W_GridSettings}}", 0.5 * ClientWidth( Editor.Window ) - 196, 0.5 * ClientHeight( Editor.Window ) - 87, 293, 175, Desktop(), WINDOW_TITLEBAR|WINDOW_RESIZABLE )
		CreateLabel( "{{L_CellWidth}}", 6, 11, 209, 16, GridSettingsWindow, Label_Right )
		CreateLabel( "{{L_CellHeight}}", 6, 37, 209, 16, GridSettingsWindow, Label_Right )
		CreateLabel( "{{L_VerticalCellDivision}}", 6, 59, 209, 16, GridSettingsWindow, Label_Right )
		CreateLabel( "{{L_HorizontalCellDivision}}", 6, 83, 209, 16, GridSettingsWindow, Label_Right )
		Local CellWidthField:TGadget = CreateTextField( 220, 8, 56, 20, GridSettingsWindow )
		Local CellHeightField:TGadget = CreateTextField( 220, 32, 56, 20, GridSettingsWindow )
		Local HorizontalDivField:TGadget = CreateTextField( 220, 56, 56, 20, GridSettingsWindow )
		Local VerticalDivField:TGadget = CreateTextField( 220, 80, 56, 20, GridSettingsWindow )
		Local SelectColorButton:TGadget = CreateButton( "{{B_SelectColor}}", 58, 112, 80, 24, GridSettingsWindow, BUTTON_PUSH )
		Local OKButton:TGadget = CreateButton( "{{B_OK}}", 146, 112, 80, 24, GridSettingsWindow, BUTTON_PUSH )
