...
Type TMario Extends LTVectorSprite
	...
	Method Init()
		...
		AttachModel( THorizontalMovement.Create( MarioCollidedWithObject, MarioCollidedWithWall ) )
 		AttachModel( TVerticalMovement.Create( MarioCollidedWithObject, MarioCollidedWithFloor ) )
...
Type TMovement Extends LTBehaviorModel
	Method ApplyTo( Shape:LTShape )
		LTSprite( Shape ).MoveForward()
	End Method
End Type



Global MarioCollidedWithObject:TMarioCollidedWithObject = New TMarioCollidedWithObject
Type TMarioCollidedWithObject Extends LTSpriteCollisionHandler
	Method HandleCollision( Sprite1:LTSprite, Sprite2:LTSprite )
		...
		Else If TEnemy( Sprite2 ) Then
			If Mario.FindModel( "TInvulnerable" ) Then
				Sprite2.AttachModel( New TKicked )
			ElseIf Mario.BottomY() < Sprite2.Y Then
				If Mario.DY > 0.0 Then
					TEnemy( Sprite2 ).Stomp()
					TScore.FromSprite( Sprite2, Mario.Combo )
					If Mario.Combo < TScore.s400 Then Mario.Combo :+ 1
					Mario.DY = Mario.HopStrength
				End If
			Else
				Mario.Damage()
			End If
...
Type TMarioCollidedWithWall Extends LTSpriteAndTileCollisionHandler
	Method HandleCollision( Sprite:LTSprite, TileMap:LTTileMap, TileX:Int, TileY:Int )
		...
		If TileNum = TTIles.Coin Then
			...
			Game.CoinFlip.Play()
		Else
			Mario.PushFromTile( TileMap, TileX, TileY )
		End If
	End Method
End Type
...
Type TDying Extends LTBehaviorModel
	...
	Method Activate( Shape:LTShape )
		Local Mario:TMario = TMario( Shape )
		Mario.DeactivateModel( "THorizontalMovement" )
		Mario.DeactivateModel( "TVerticalMovement" )
		Mario.DeactivateModel( "TMoving" )
		Mario.DeactivateModel( "TJumping" )
		Mario.AttachModel( New TMovement )
		Mario.DX = 0.0
		If Not Chasm Then
...
Type TFinalSequence Extends LTBehaviorModel
	...
	Method Activate( Shape:LTShape )
		...
		Game.Level.Active = False
		Mario.X = Pole.X - 0.3 * Mario.GetFacing()
		Mario.Frame = TMario.SlidingDown
		Game.MusicChannel.SetVolume( 0.0 )
		Game.FlagPole.Play()
		TScore.FromSprite( Mario, L_LimitInt( ( Pole.BottomY() - Mario.BottomY() ) / Pole.Height * 11, TScore.s100, TScore.s8000 ) )
	End Method
	
	Method ApplyTo( Shape:LTShape )
		Select Phase
			Case Sliding
				If Mario.BottomY() < Pole.BottomY() Then Mario.Move( 0.0, MarioSpeed )
				If Flag.BottomY() < Pole.BottomY() Then
					Flag.Move( 0.0, FlagSpeed )
				ElseIf Mario.BottomY() >= Pole.BottomY() Then
					NextPhase( Shape )
				End If
			Case Walking
				Mario.DX = WalkingSpeed
				If Mario.OnLand Then
					Mario.Animate( WalkingAnimationSpeed, 3, 1, AnimationStartingTime )
				Else
					AnimationStartingTime = Game.Time
				End If
				If Mario.X >= FinalExit.X Then NextPhase( Shape )
			Case Exiting
				Mario.DX = WalkingSpeed
				If Mario.LeftX() >= FinalExit.RightX() Then NextPhase( Shape )
			Case RaisingFlag
	...
	Method NextPhase( Shape:LTShape )
		...
		Select Phase
			...
			Case Exiting
				Mario.LimitByWindowShape( FinalExit )
			Case RaisingFlag
				Mario.Visible = False
				Mario.DX = 0.0
