...
Function ResizeTilemap( TileMap:LTTileMap )
	...
	Form.NewLine()
	Local NewXQuantityTextField:TGadget = Form.AddTextField( "{{L_HorizontalCells}}", 180 )
	SetGadgetText( NewXQuantityTextField, XQuantity )
	ActivateGadget( NewXQuantityTextField )
	Local NewYQuantityTextField:TGadget = Form.AddTextField( "{{L_VerticalCells}}", 180 )
	...
	Repeat
		...
		Local XScale:Float = 1.0 * TilemapsCanvasSize / ( MaxX - MinX )
		Local YScale:Float = 1.0 * TilemapsCanvasSize / ( MaxY - MinY )
		Local Scale:Float = Min( XScale, YScale )
